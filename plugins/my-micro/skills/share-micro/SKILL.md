...
For deletion, run `scripts/run.sh delete-draft --post <id> --service <origin> --out <new-draft-file>`. This uses an owner-only read, including posts hidden by moderation. Show the target URL, title, and version, explicitly ask to delete that post, then use the same `publish --draft ... --confirm ...` command. The server requires the confirmed version so a changed post is not silently deleted. Hidden posts remain unavailable for public viewing or updates.
