...
4. Ask for explicit confirmation to publish the displayed content to the displayed service. The local draft's `approvalHash` binds the exact content, operation, target, and destination. Do not infer approval from installing the plugin, signing in, or a broad request to create a preview. A user request that explicitly approves this exact already displayed draft is sufficient.
...
Only after the displayed draft has been approved, run `scripts/run.sh publish --draft <draft-file> --confirm <approvalHash-from-preview>`. The CLI sends the saved snapshot and verifies the stored result by reading it back. Return its public page URL after verification succeeds.

- Never recollect settings, edit the saved draft, or change its title, destination, or target between preview and submission. Changes require a new draft, a complete preview, and approval again.
