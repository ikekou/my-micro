...
Micro本体はCSSで描画し、キーの図柄はCodexアプリ内のSVGを元にしています。キー素材の出典と扱いは `apps/web/app/assets/micro-keycaps/README.md` に記載し、My MicroのMITコードライセンスと区別します。My Microは非公式の共有サイトであること、公開データと認証データの範囲、削除・問い合わせ方法をサイト内に表示します。
