...
interface SessionInfo { id: string; createdAt: string; expiresAt: string; userAgent: string | null; current: boolean }
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const { sessions } = await apiData<{ sessions: SessionInfo[] }>(request, "/api/v1/me/sessions");
...
export default function Account() {
  ...
  async function revoke(id: string) {
    ...
    try { await clientApi(`/api/v1/me/sessions/${encodeURIComponent(id)}`, { method: "DELETE" }); setRemoved([...removed, id]); setConfirming(null); }
